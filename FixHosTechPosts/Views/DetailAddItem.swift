...
struct DetailAddItem: View {
    ...
    var body: some View {
        HStack {
            ...
            Button(action: {
                self.userData.addItem(elementIndex:self.elementIndex,医师组名称: self.医师组名称,项目名称: self.itemName, 每次所需分钟: self.minutesPerCase, 年总次数: self.casesPerYear, 配备技师数: self.techsPerItem, 配备护士数: self.nursesPerItem, 配备医师数: self.doctorsPerItem, 备注: self.comment)
            }) {
                Text("新增诊疗项目")
            }
        }
